class ToursController < InheritedResources::Base
  def show
    ...
    @related = Tour.search(params.merge({ region: regions_ids, tour_typ: @tour.tour_types.map(&:id) })).where('tours.id != ?', @tour.id).limit(6)
    @random = Tour.find(Tour.where('tours.id != ?', @tour.id).pluck(:id).sample(6))
  end
